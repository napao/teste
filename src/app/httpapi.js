(function (App) {
	...
	var initServer = function () {
		...
		server.expose('getgenres', function (args, opt, callback) {
			...
			case 'shows':
				popcornCallback(callback, false, {
					'genres': Settings.get('genres_tv')
				});
				break;
			case 'anime':
				popcornCallback(callback, false, {
					'genres': Settings.get('genres_anime')
				});
				break;
			case 'movies':
				popcornCallback(callback, false, {
		...
		server.expose('getsorters', function (args, opt, callback) {
			...
			case 'anime':
				popcornCallback(callback, false, {
					'sorters': Settings.get('sorters_tv')
				});
				break;
			case 'movies':
				popcornCallback(callback, false, {
					'sorters': Settings.get('sorters')
				});
				break;
			default:
				popcornCallback(callback, false, {
		...
		server.expose('gettypes', function (args, opt, callback) {
			...
			case 'anime':
				popcornCallback(callback, false, {
					'types': Settings.get('types_anime')
				});
				break;
			default:
				popcornCallback(callback, false, {
		...
		server.expose('listennotifications', function (args, opt, callback) {
			...
			Vent.on('player:pause', playingChange);
			Vent.on('player:play', playingChange);
			...
			Vent.on('viewstack:push', emitViewChange);
			Vent.on('viewstack:pop', emitViewChange);
		});
	};
	...
	function startListening() {
		httpServer = server.listen(Settings.get('httpApiPort'));
	...
	function popcornCallback(callback, err, result) {
		...
		result['popcornVersion'] = Settings.get('version');
		callback(err, result);
	}

	Vent.on('initHttpApi', function () {
		console.log('Reiniting server');
		Q.fcall(initServer)
			.then(function () {
				server.enableAuth(Settings.get('httpApiUsername'), Settings.get('httpApiPassword'));
				if (httpServer) {
