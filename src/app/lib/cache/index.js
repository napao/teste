(function (App) {
	...
	function Cache(table) {
		...
		this.table = table;
		var db = indexedDB.open(Settings.get('cachev2').name, Settings.get('cachev2').version);
		...
		db.onupgradeneeded = function () {
			var self = this;
			Settings.get('cachev2').tables.forEach(function (tableName) {
	...
	App.CacheV2.deleteDatabase = function () {
		...
		return Q.Promise(function (resolve, reject) {
			indexedDB.deleteDatabase(Settings.get('cachev2').name);
			resolve();
		});
	};
