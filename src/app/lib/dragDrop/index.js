(function (App) {
	...
	function onDrop(e) {
		...
		if (file != null && (file.name.indexOf('.torrent') !== -1 || file.name.indexOf('.srt') !== -1)) {
			...
			reader.onload = function (event) {
				...
				fs.writeFile(path.join(Settings.get('tmpLocation'), file.name), content, function (err) {
					if (err) {
						window.alert('Error Loading File: ' + err);
					} else {
						if (file.name.indexOf('.torrent') !== -1) {
							var torrentsrc = path.join(Settings.get('tmpLocation'), file.name);
							...
							Settings.set('droppedSub', file.name);
							Vent.trigger('videojs:drop_sub');
						}
					}
				});
			};
	...
	function onPaste(e) {
		...
		if (torrentsrc.indexOf('magnet') > -1) {
			...
			var ws = fs.createWriteStream(path.join(Settings.get('tmpLocation'), 'pct-remote-torrent.torrent'));

			if (fs.exists(path.join(Settings.get('tmpLocation'), 'pct-remote-torrent.torrent'))) {
				fs.unlink(path.join(Settings.get('tmpLocation'), 'pct-remote-torrent.torrent'));
			}
