	...
	var Loading = Backbone.Marionette.ItemView.extend({
			...
			if (streamInfo) {
				...
				if (percent > 99) {
					...
					if (this.player === 'local') {
						...
						Vent.trigger('stream:local', playerModel);
					} else {

						Vent.trigger('stream:ready', that.model.get('player'));
					}
		...
		augmentDropModel: function (data) {
			...
			case 'dropped-tvshow':
				...
				Providers.get('tvshow-metadata').episodeSummary(metadata.showName, metadata.season, metadata.episode).then(function (data) {
					if (!data) {
			...
			case 'dropped-movie':
				// @TODO: REMOVE THAT AND MIGRATE IT TO A PACKAGE !!!
				Providers.get('movie-metadata').findSummary(metadata.title).then(function (data) {
					if (!data) {
						win.warn('Unable to fetch data from Trakt.tv');
					} else {
						data = data[0];
