	...
	var Player = Backbone.Marionette.ItemView.extend({
		...
		closePlayer: function () {
			...
			if (type !== 'trailer' || !(data.type.indexOf('dropped') > -1)) {
				if (this.video.currentTime() / this.video.duration() >= 0.8) {
					Vent.trigger(type + ':watched', this.model.attributes, 'scrobble');
				} else {
					var metatype = type + '-metadata';
					try {
						Providers.get(metatype).cancelWatching();
					} catch (err) {
						console.log(err);
					}
		...
		updateWatching: function () {
			if (this.model.get('type') === 'movie') {
				...
				Providers.get('movie-metadata').watching(this.model.get('metadata').imdb_id, this.video.currentTime() / this.video.duration() * 100 | 0, this.video.duration() / 60 | 0);
			} else {
				...
				Providers.get('tvshow-metadata').watching(this.model.get('metadata').tvdb_id, this.model.get('metadata').season, this.model.get('metadata').episode, this.video.currentTime() / this.video.duration() * 100 | 0, this.video.duration() / 60 | 0);
			}
		},
				...
				if ((this.video.duration() - this.video.currentTime()) < 60 && this.video.currentTime() > 30) {
					...
					if (!this.autoplayisshown) {
						...
						if (!this.precachestarted) {
							Vent.trigger('preload:start', this.next_episode_model);
		...
		playNextNow: function () {
			...
			Vent.trigger('preload:stop');
			Vent.trigger('player:close');
			Vent.trigger('stream:start', this.next_episode_model);
