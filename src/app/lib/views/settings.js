(function (App) {
	...
	var PackagesManager = require('./lib/api').packages.manager;
	var ThemesManager = require('./lib/api').themes;
	...
	var SettingsView = Backbone.Marionette.ItemView.extend({
		...
		templateHelpers: {
			...
			screens: function () {
				return ['Favorites', 'Watchlist', 'Last Open'];
			},
			...
			languages: function () {
				var languages = [];
				for (var key in Localization.allTranslations) {
					key = Localization.allTranslations[key];
					if (Localization.langcodes[key] !== undefined) {
						languages.push(key);
					}
				}
				return languages;
			...
			langCode: function () {
				...
				for (var key in Localization.langcodes) {
					if (Localization.langcodes[key].subtitle !== undefined && Localization.langcodes[key].subtitle === true) {
						languages.push(key);
					}
				}
				return languages;
			...
			cssFiles: function () {
				return ThemesManager.getAllCss();
			},
		...
		renderPackageSettings: function () {
			...
			var self = this;
			var settingPackages = [];
			var loadedPackages = PackagesManager.getLoadedPackages();
		...
		onRender: function () {
			if (Settings.get('showAdvancedSettings')) {
		...
		clickButtonPackage: function (e) {
			...
			var thisPackage = PackagesManager.getLoadedPackage(field.attr('data-package'));
		...
		saveSetting: function (e) {
			...
			case 'start_screen':
				if ($('option:selected', field).val() === 'Last Open') {
					Settings.set('lastTab', App.currentview);
				}
				/* falls through */
			case 'watchedCovers':
			case 'theme':
			...
			Settings.set(field.attr('name'), value);

			if (apiDataChanged) {
				Vent.trigger('initHttpApi');
			}
		...
		syncSetting: function (setting, value) {
			...
			case 'theme':
				...
				Settings.set('activeCss', false); // reset css
				that.alertMessageSuccess(true);
				break;
			case 'css':
				Settings.set('activeCss', value);
				...
				break;
			case 'start_screen':
				Settings.set('startScreen', value);
				break;
			default:
		...
		openDatabaseFolder: function () {
			console.log('Opening: ' + Settings.get('databaseLocation'));
			gui.Shell.openItem(Settings.get('databaseLocation'));
		},
		...
		exportDatabase: function (e) {
			...
			var zip = new AdmZip();
			var btn = $(e.currentTarget);
			var databaseFiles = fs.readdirSync(Settings.get('databaseLocation'));

			databaseFiles.forEach(function (entry) {
				zip.addLocalFile(Settings.get('databaseLocation') + '/' + entry);
			});
		...
		importDatabase: function () {
			...
			fdialogs.readFile(function (err, content, path) {
				...
				try {
					...
					zip.extractAllTo(Settings.get('databaseLocation') + '/', /*overwrite*/ true);
					that.alertMessageSuccess(true);
				} catch (err) {
