	...
	var ShowDetail = Backbone.Marionette.ItemView.extend({
		...
		templateHelpers: {
			...
			seasons: function (options) {
				return _.uniq(_.pluck(this.episodes, 'season')).sort(function (a, b) { return a - b; });
			},
		...
		toggleFavorite: function (e) {
			...
			if (bookmarked !== true) {
				...
				var provider = Providers.get(this.model.get('provider'));
				provider.detail(this.model.get('imdb_id'), this.model.attributes)
					.then(function (data) {

						Database.add('tvshows', data)
							.then(function () {
								App.userBookmarks.push(that.model.get('imdb_id'));
								return Database.add('bookmarks', {
				...
				Database.delete('bookmarks', {
					...
					.then(function () {
						...
						return Database.delete('tvshows', {
							imdb_id: that.model.get('imdb_id')
						});
					})
					.then(function () {
		...
		initialize: function () {
			...
			Vent.on('viewstack:pop', function () {
			...
			Vent.on('viewstack:push', function () {
				if (_.last(App.ViewStack) !== _this.className) {
					_this.unbindKeyboardShortcuts();
				}
			});
			Vent.on('show:watched:' + this.model.id,
				_.bind(this.onWatched, this));
			Vent.on('show:unwatched:' + this.model.id,
			...
			Vent.on('shortcuts:show', function () {
				_this.initKeyboardShortcuts();
			});
		},
		...
		onShow: function () {
			...
			if (Settings.get('ratingStars') === false) {
			...
			Database.find('watched', {
				...
				.then(function (data) {
					...
					if (episodesSeen.length > 0) {
						...
						if (Settings.get('tv_detail_jump_to') !== 'firstUnwatched') {
				...
				.then(function (watched) {
					if (watched) {
						...
						Database.find('watched', {
							...
							.then(function () {
								return Database.delete('watched', {
									tvdb_id: currentValue.tvdb_id.toString(),
									imdb_id: currentValue.imdb_id.toString(),
									season: currentValue.season.toString(),
									episode: currentValue.episode.toString()
							...
							.then(function () {
								Vent.trigger('show:unwatched:' + currentValue.tvdb_id, currentValue);
							});
						...
						Database.find('watched', {
							...
							.then(function () {
								Vent.trigger('show:watched:' + currentValue.tvdb_id, currentValue);
							});
					}
				});
		...
		startStreaming: function (e) {
			...
			if (Settings.get('playNextEpisodeAuto')) {
			...
			_this.unbindKeyboardShortcuts();
			Vent.trigger('streamer:start', torrentStart);
		},
